...
class Ledger {
     ...
     */
    async getOneBy(args)  {
        return await this.qldbConnection.findOneBy(this.tableName, this.model, args);
    };
    ...
    async add(data, maxDepth=3)  {
        const mappedModel = await this.mapDataToModel(data, this.model, 0, maxDepth);
    ...
    async update(args, maxDepth=3) {
        const mappedModel = await this.mapDataToModel(args.fields, this.model, 0, maxDepth, true);
        if (mappedModel.errors) {
            return {errors: mappedModel.errors};
        }
        if (this.options.timestamps == true){
     ...
     */
    async mapDataToModel(data, originalModel, currentDepth = 0, maxDepth = 3, isUpdate = false) {
        const model = _.cloneDeep(originalModel);
