...
const { toCamelCase } = require('./utils/case-convert');
...
class Ledger {
    constructor(qldbConnection, tableName, model, options) {
        ...
        this.options.toCamelCase = !!this.options.toCamelCase;
        ...
        // instance methods
        this.instanceMethod = {};
        if (this.options.instanceMethod) {
            this.instanceMethod = this.options.instanceMethod;
        }
    ...
    bindInstanceMethod(modelObject) {
        Object.entries(this.instanceMethod).forEach(([k, v]) => {
            Object.assign(modelObject, { [k]: v.bind(modelObject) });
        });
    ...
    buildModelObject(raw) {
        ...
        if (this.options.toCamelCase) {
            result = toCamelCase(raw);
        }

        return bindInstanceMethod(result);
    } 
    ...
    async getBy(args)  {
        ...
        return results.map((res) => this.buildModelObject(res));
    };
            ...
            if (data[fieldName]) {
                ...
                if (fieldType == 'ledger') { // Check the values in the linked model
                    ...
                    if (typeof data[fieldName] != 'object') {
                        ...
                        model[fieldName].value = result[0].metadata.id;
