...
class qldbConnect {
    ...
    constructor(ledgerName, serviceConfigOptions) {
        this.qldbDriver = new PooledQldbDriver(ledgerName, serviceConfigOptions, 2, undefined ,20);
    ...
    async buildSqlUpdateRecursive(tableName, args, model) {
        ...
        for (const [fieldName, fieldValue] of Object.entries(args.fields)) {
            ...
            if (fieldType == 'ledger') {
                let { pkName, pkValue } = await this.getLinkedLedgerPk(tableName, fieldName, model, args.where);
                const subArgs = {
                    fields: fieldValue,
                    where: {}
                }
                ...
                const newValue = JSON.stringify(fieldValue).replace(/"/ig, "'")
                ...
                const sqlRecurssive = await this.buildSqlUpdateRecursive(currentLayer, {fields: fieldValue}, model[fieldName].model);
                sqlUpdate = sqlUpdate + sqlRecurssive;
            } else {
                sqlUpdate = sqlUpdate + tableName + "." + fieldName + " = '" + fieldValue + "' , ";
            }
        }
    ...
    async buildSqlDelete(tableName, model, args) {
        ...
        for (const [fieldName, fieldValue] of Object.entries(model)) {
            ...
            if (fieldType == 'ledger') {
                ...
                const { pkName, pkValue } = await this.getLinkedLedgerPk(tableName, fieldName, model, args.where)
                let subArgs = {
                    recursive: recursive,
                    where: []
                };
    ...
    async getLinkedLedgerPk(tableName, fieldName, model, args) {
        let sqlWhere = '';
        for (const [fieldName, fieldValue] of Object.entries(args)) {
            sqlWhere += fieldName + '= \''+ fieldValue +'\' AND ';
        }
        let sql = 'SELECT '+ fieldName +' FROM '+ tableName +' WHERE '+ sqlWhere.substr(0, sqlWhere.length - 4) +' ;';
        const linkedLedger = await this.executeRawSQL(sql);

        sql = 'SELECT * FROM _ql_committed_'+ model[fieldName].model.tableName +' WHERE metadata.id = \''+ linkedLedger[0][fieldName] +'\';';
        const linkedLedgerData = await this.executeRawSQL(sql);
        const pkName = model[fieldName].model.primaryKey;
        const pkValue = linkedLedgerData[0].data[pkName];
        ...
        if (args.where) {
            for (let [key, value] of Object.entries(args.where)) {
                ...
                if ((typeof(value) == 'array') || (typeof(value) == 'object')) {
                    ...
                    sqlWhere = sqlWhere + tableName + "." + key + operator.operator + "[" + sendValues.join(',') + "] AND ";
                } else if(typeof(value) === 'number') {
                    sqlWhere = sqlWhere + tableName + "." + key + operator.operator + value + " AND ";
                } else {
                    sqlWhere = sqlWhere + tableName + "." + key + operator.operator + "'" + value + "' AND ";
                }
            }
    ...
    async buildDoc(model, data = false) {
        ...
        for (const [fieldName, fieldOptions] of Object.entries(model)) {
            ...
            if (fieldType == 'ledger') { // process the values in the linked model
                const documentId = await this.processLedgerData(fieldOptions.model, model[fieldName].value);
                doc[fieldName] = documentId;
            } else if (fieldType == 'object') {
    ...
    async processLedgerData(ledgerModel, value) {
        ...
        let result = await this.create(ledgerModel.tableName, ledgerModel.model);
