...
class qldbConnect {
    ...
    async getIndexes(tableName) {
        const statement = `SELECT indexes FROM information_schema.user_tables WHERE name = '${tableName}' AND status = 'ACTIVE'`;
    ...
    async checkIfIndexExists(tableName, indexName) {
        try {
            ...
            if (tableIndexes) {
                ...
                if (!indexList.includes(`[${indexName.toUpperCase()}]`)) {
  ...
  async findCommittedDocument(tableName, args) {
    ...
    async buildSqlUpdateRecursive(tableName, args, model) {
        ...
        for (const [fieldName, fieldValue] of Object.entries(args.fields)) {
            ...
            } else if (['number', 'int', 'boolean'].includes(fieldType)) {
 ...
 */
  async buildCommittedDocumentMetaSqlSelect(tableName, args) {
    ...
    async buildSqlInsert(tableName, model) {
        ...
        const insertValues = JSON.stringify(doc);
        return 'INSERT INTO ' + tableName + ' `' + insertValues + '`';
    }
